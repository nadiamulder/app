...
# --- Optional METs Calculator Sidebar or Section ---
with st.expander("🏃 Optional: METs Calculator for Athletes"):
    try:
        mets_df = pd.read_csv("METS_Data_Unique.csv")
        import re
        mets_df["MET"] = mets_df["value"].apply(lambda x: float(re.search(r"[\d\.]+", str(x)).group()) if re.search(r"[\d\.]+", str(x)) else None)
        mets_df = mets_df[["description", "MET"]].dropna()

        selected_activity = st.selectbox("Choose activity", ["None"] + mets_df["description"].tolist())
        duration_mins = st.number_input("Duration (minutes)", min_value=0, value=0, key="mets_duration")

        mets_kcal = 0
        if selected_activity != "None" and duration_mins > 0:
            met_value = mets_df[mets_df["description"] == selected_activity]["MET"].values[0]
            mets_kcal = (met_value * weight * (duration_mins / 60))
            st.info(f"Estimated calories burned from activity: {mets_kcal:.0f} kcal")

        st.session_state["mets_kcal"] = mets_kcal
    except:
        st.warning("Could not load METs activity data.")

# --- Optional Energy Adjustments ---
with st.expander("➕ Additional TDEE Adjustments"):
    preg_add = st.number_input("Pregnancy/Breastfeeding (kcal)", value=0, step=50, key="preg")
    goal_adjust = st.number_input("Weight Goal Adjustment (kcal)", value=0, step=50, key="goal")
    custom_adjust = st.number_input("Custom kcal Adjustment (any reason)", value=0, step=50, key="custom")

    # Combine all adjustments
    extra_kcal = preg_add + goal_adjust + custom_adjust + st.session_state.get("mets_kcal", 0)
    st.markdown(f"**Total Adjustments: +{extra_kcal:.0f} kcal**")

    # Update final TDEE if original TDEE is calculated
    try:
        final_tdee = tdee + extra_kcal
        st.success(f"Final Adjusted TDEE: {final_tdee:.0f} kcal/day")
        st.session_state["final_tdee"] = final_tdee
    except:
        st.warning("Base TDEE not found. Please complete personal data section first.")
