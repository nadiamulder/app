...
# --- Load All Exchange Category CSVs ---
...
with st.expander("4️⃣ Meal Plan Builder", expanded=True):
    st.subheader("🍽️ Create Daily Meal Plan")
    ...
    for meal in meals:
        ...
        selected_items = {}
        cols = st.columns(3)
        for i, (category, df) in enumerate(exchange_categories.items()):
            with cols[i % 3]:
                count = st.number_input(f"{meal} – {category} servings", min_value=0, max_value=5, key=f"{meal}_{category}")
                if count > 0:
                    options = df["Food Item"].dropna().sample(min(len(df), 10)).tolist()
                    items = st.multiselect(f"{meal} – Choose {category} items", options, max_selections=count, key=f"{meal}_{category}_items")
                    selected_items[category] = items
        meal_plan[meal] = selected_items

    st.markdown("---")
    st.markdown("## 🧾 Meal Plan Summary")

    for meal, contents in meal_plan.items():
        st.markdown(f"**{meal}**")
