...
def load_exchange_data():
    try:
        ...
        st.error("Could not load combined exchange data.")
        return pd.DataFrame()
...
with st.expander("📊 Step 3: Exchange Planner", expanded=True):
    ...
    exchange_df_tally = pd.DataFrame(macro_totals, index=["From Exchanges"]).T
    st.dataframe(exchange_df_tally.style.format("{:.0f}"))
