...
# --- METs Calculator (Optional Info for Athletes) ---
with st.expander("🏃 METs Calculator (Optional Info for Athletes)"):
    try:
        mets_df = pd.read_csv("Cleaned_METS.csv")
        ...
        st.session_state["final_tdee"] += mets_kcal
        st.success(f"Updated Final TDEE (with METs): {st.session_state['final_tdee']:.0f} kcal/day")
